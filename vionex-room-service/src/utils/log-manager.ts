...
export class LogManager {
    ...
    /**
     * Initialize log file with date_room_service format
     */
    private initializeLogFile(): void {
        try {
            ...
            // Generate filename: YYYY-MM-DD_room_service.log (one file per day)
            const date = new Date();
            const dateStr = date.toISOString().split('T')[0]; // YYYY-MM-DD
            const fileName = `${dateStr}_room_service.log`;
            ...
            if (!fileExists) {
                ...
                this.writeLine(
                    'log-manager.ts',
                    `=== Room Service Log File Created at ${timestamp} ===`,
                );
                this.writeLine(
                    'log-manager.ts',
    ...
    public close(): void {
        if (this.logStream) {
            this.writeLine(
                'log-manager.ts',
                '=== Room Service Log Session Ended ===',
            );
            this.logStream.end();
