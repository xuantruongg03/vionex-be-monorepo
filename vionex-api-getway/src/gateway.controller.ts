...
export class GatewayController {
  ...
  async verifyRoomPassword(@Body() data: { roomId: string; password: string }) {
    ...
    try {
      ...
      if (!isLocked) {
        return {
          data: { success: true, locked: false, message: 'Room is not locked' },
        };
      }
...
//   }
  ...
  // HTTP endpoint for updating stream metadata
  ...
  // HTTP endpoint for unpublishing stream
  ...
  async removeUser(
    ...
    try {
      ...
      const requester = await this.getParticipantFromHeader(authorization);
      ...
      if (leaveRoomResponse?.data?.newCreator) {
        this.broadcastService.broadcastToRoom(roomId, 'sfu:creator-changed', {
  ...
  // HTTP endpoint for pausing consumer
  @Post('sfu/consumer/:consumerId/pause')
  async pauseConsumer(
    @Param('consumerId') consumerId: string,
    @Body() data: { roomId: string },
    @Headers('authorization') authorization?: string,
    ...
    try {
      ...
      // Consumer pause feature not implemented
      console.log('Consumer pause feature not implemented');

      return {
        success: false,
        message: 'Consumer pause feature not implemented',
      };
    } catch (error) {
      console.error('Error pausing consumer:', error);
      throw new HttpException(
        error.message || 'Failed to pause consumer',
        error.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  // HTTP endpoint for closing consumer
  @Delete('sfu/consumer/:consumerId')
  async closeConsumer(
    @Param('consumerId') consumerId: string,
    @Body() data: { roomId: string },
    @Headers('authorization') authorization?: string,
    ...
    try {
      ...
      // Consumer close feature not implemented
      console.log('Consumer close feature not implemented');

      return {
        success: false,
        message: 'Consumer close feature not implemented',
      };
    } catch (error) {
      console.error('Error closing consumer:', error);
      throw new HttpException(
        error.message || 'Failed to close consumer',
        error.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  // HTTP endpoint for pausing producer
  @Post('sfu/producer/:producerId/pause')
  async pauseProducer(
    ...
    try {
      ...
      // Producer pause feature not implemented
      console.log('Producer pause feature not implemented');

      return {
        success: false,
        message: 'Producer pause feature not implemented',
      };
    } catch (error) {
      console.error('Error pausing producer:', error);
      throw new HttpException(
        error.message || 'Failed to pause producer',
        error.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  // HTTP endpoint for resuming producer
  @Post('sfu/producer/:producerId/resume')
  async resumeProducer(
    ...
    try {
      ...
      console.log('Producer resume feature not implemented');
      ...
      throw new HttpException(
        error.message || 'Failed to resume producer',
        error.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
