...
interface VotingServiceClient {
  ...
  endVote(data: { room_id: string; vote_id: string; creator_id: string }): any;
...
interface QuizServiceClient {
  ...
  endQuiz(data: { room_id: string; quiz_id: string; creator_id: string }): any;
...
export class InteractionClientService implements OnModuleInit {
  ...
  onModuleInit() {
    this.whiteboardService =
      this.client.getService<WhiteboardServiceClient>('WhiteboardService');
    this.votingService =
      this.client.getService<VotingServiceClient>('VotingService');
    this.quizService = this.client.getService<QuizServiceClient>('QuizService');
  ...
  async getWhiteboardData(roomId: string) {
    try {
      ...
      if (result?.success && result?.whiteboard_data?.elements) {
        const deserializedElements = result.whiteboard_data.elements.map(
          (element) => {
            try {
              // Parse the data field back to object
              const parsedData = element.data ? JSON.parse(element.data) : {};
              ...
              console.error('[InteractionClient] Error parsing element data:', {
      ...
      console.error('[InteractionClient] getWhiteboardData error:', error);
      throw error;
    }
  }
  ...
  async checkUserPermission(
    ...
    try {
      ...
      console.error('[InteractionClient] checkUserPermission error:', error);
      return {
        success: false,
        can_draw: false,
      };
  ...
  async getPointers(roomId: string) {
    const observable = this.whiteboardService.getPointers({ room_id: roomId });
    return await firstValueFrom(observable);
  }
  ...
  async getVoteResults(roomId: string, voteId: string) {
    try {
      ...
      console.error('[InteractionClient] Error getting vote results:', error);
      throw error;
    }
  }
  ...
  async getActiveVote(roomId: string) {
    try {
      const observable = this.votingService.getActiveVote({ room_id: roomId });
      const result = await firstValueFrom(observable);

      return result;
    } catch (error) {
      console.error('[InteractionClient] Error getting active vote:', error);
      throw error;
    }
  }
  ...
  async getQuizResults(roomId: string, quizId: string) {
    try {
      ...
      console.error('[InteractionClient] Error getting quiz results:', error);
      throw error;
    }
  }
  ...
  async getActiveQuiz(roomId: string) {
    try {
      ...
      console.error('[InteractionClient] Error getting active quiz:', error);
      throw error;
    }
  }
  ...
  async saveUserBehavior(
    ...
    try {
      ...
      console.error('[InteractionClient] Error saving user behavior:', error);
      throw error;
    }
  }
  ...
  async clearRoomLogs(roomId: string) {
    try {
      ...
      console.error('[InteractionClient] Error clearing room logs:', error);
      throw error;
    }
  }
}
