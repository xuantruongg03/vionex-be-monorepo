...
export class LogManager {
    ...
    /**
     * Initialize log file with date_serviceName format (one file per day per service)
     */
    private initializeLogFile(): void {
        try {
            ...
            // Generate filename: YYYY-MM-DD_serviceName.log (one file per day)
            const date = new Date();
            const dateStr = date.toISOString().split('T')[0]; // YYYY-MM-DD
            const fileName = `${dateStr}_${this.serviceName}.log`;
            ...
            if (!fileExists) {
                ...
                this.writeLine(
                    'log-manager.ts',
                    `=== ${this.serviceName.toUpperCase()} Service Log File Created at ${timestamp} ===`,
                );
                this.writeLine(
                    'log-manager.ts',
    ...
    /**
     * Write a log line with format: [lineNumber] [timestamp] [fileName] message
     */
...
// Export logger instance for SFU service
export const logger = new LogManager('sfu');
